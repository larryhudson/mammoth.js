...
function BodyReader(options) {
  ...
  function readDrawingElement(element) {
    ...
    var resultsToReturn = combineResults(
      blips.map(readBlip.bind(null, element)),
      rawHeight,
      rawWidth
    );
  ...
  function readImage(imageFile, altText, relationshipId) {
...
function combineResults(results, rawHeight = null, rawWidth = null) {
  var result = Result.combine(_.pluck(results, "_result"));
  return new ReadResult(
    _.flatten(_.pluck(result.value, "element")),
    _.merge(_.filter(_.flatten(_.pluck(result.value, "extra")), identity), {
      rawHeight,
      rawWidth
    }),
    result.messages
